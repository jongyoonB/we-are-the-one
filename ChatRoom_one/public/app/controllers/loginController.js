...
.controller('loginCtrl', function ($scope, $location, $rootScope, $socket, $routeParams){		// Login Controller
	...
	if($rootScope.loggedIn){
		$location.path('/v1/ChatRoom/'+$routeeParams.room);
	}

	// Functions for controlling behaviour.
	$scope.redirect = function(){
var nick = $("#u_nick").val();
var pic = $("#u_pic").attr('src');
var room = $("#room").val();
$scope.username=nick;
$scope.userAvatar=pic;
$scope.room = room;
if ($scope.username.length <= 20) {
			if($scope.username){
				$socket.emit('new user',{username : $scope.username, userAvatar : $scope.userAvatar, roomID : $scope.room},function(data){
					//if(data.success == true){	// if nickname doesn't exists
						$rootScope.username = $scope.username;
						$rootScope.userAvatar = $scope.userAvatar;
						$rootScope.loggedIn = true;
						$location.path('/v1/ChatRoom/'+$routeParams.room);
