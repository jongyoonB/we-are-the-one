...
.controller('chatRoomCtrl', function ($scope, $rootScope, $routeParams, $socket, $location, $http, Upload, $timeout, sendImageService){		// Chat Page Controller
	...
	if(!$rootScope.loggedIn){
		$location.path('/v1/'.$routeParams.room);
	}
// ================================== Online Members List ===============================
	$socket.emit('get-online-members',{room : $routeParams.room});
...
//			var log = '{"user" : "' + $rootScope.username + '", "avatar" : "' + $rootScope.userAvatar + '", "msgTime" : "' + dateString + '", "msg" : "' + $scope.chatMsg + '"}'
var log = '{"user" : "' + $rootScope.username + '", "avatar" : "' + $rootScope.userAvatar + '", "msgTime" : "' + dateString + '", "msg" : "' + $scope.chatMsg + '"'+"\n"+'}';
		
			$socket.emit("send-message",{ username : $rootScope.username, userAvatar : $rootScope.userAvatar, msg : $scope.chatMsg, hasMsg : $scope.isMsg , hasFile : $scope.isFileSelected , msgTime : dateString, log : log }, function(data){
    ...
    $scope.sendImage = function (files) {
        if (files && files.length) {
            ...
            for (var i = 0; i < files.length; i++) {
				...
				$http.post($rootScope.baseUrl +"/v1/uploadImage/"+$routeParams.room, fd, {
